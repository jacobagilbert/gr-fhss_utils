...
void cf_estimate_impl::pdu_handler(pmt::pmt_t pdu)
{
    ...
    if (d_method == MIDDLE_OUT) {
        meas_err |= middle_out(mags2, bin_size, noise_floor_db, bandwidth, shift);
    } else {
        meas_err |= rms_bw(mags2, freq_axis, center_frequency, bandwidth);
        if (d_method == RMS) {
            meas_err |= rms_cf(mags2, freq_axis, center_frequency, sample_rate, shift);
        } else if (d_method == HALF_POWER) {
            meas_err |= half_power_cf(mags2, shift);
        } else {
...
bool cf_estimate_impl::half_power_cf(const std::vector<float> &mags2, float &shift)
{
