      ...
      d_thresh_min(-25.0)
{
    ...
    if (d_channel_freqs.size() == 0 && d_method == COERCE) {
        d_logger->warn("CF Estimator operating in COERCE mode with empty channel "
                       "frequency list; no CF correction will be applied!");
    }
}
...
void cf_estimate_impl::pdu_handler(pmt::pmt_t pdu)
{
    ...
    if (burst_size < MIN_BURST_SIZE) {
        d_logger->info("Burst of length {} too small (min = {}), dropping.", burst_size, MIN_BURST_SIZE);
    }
